...
python main.py                   # 运行
pytest tests/ -q                 # 测试（当前 152 个，含 1 个 ~12s 的 slow 真实图验收）
...
- **解码 worker 生命周期由模块级 `_ACTIVE_WORKERS` 注册表持有（D37）+ `atexit` 退出兜底（D42）**：不得挂回窗口实例属性；解释器退出路径（异常/sys.exit）由 atexit 先等 worker 收尾再回收全局
