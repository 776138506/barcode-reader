...
更新于：2026-07-24（本轮：CI/CD 全链路打通——三平台测试全绿 + v0.2.0 Release 三平台产物已发布）

## 当前状态

- **测试**:152 passed（`pytest tests/ -q`，约 25s，含 1 个 ~12s slow 真实图验收）;offscreen 冒烟 `tests/smoke_gui.py` 连跑 5 次全 OK;`tests/test_requirements.py` 守护依赖声明完整性;`tests/conftest.py` autouse 逐测 GC（防隐藏窗口引用环累积致 offscreen 崩溃，D30）
- **CI 修复（待下轮验证）**:macOS smoke 竞态「Signal source has been deleted」——根因为 CI 慢机解码超时（>30s）触发断言失败、解释器关闭回收注册表；修复为 atexit 兜底 + main 优雅停机 + smoke 超时 240s（D42）
