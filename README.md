...
从 GitHub Releases 下载**对应你系统**的产物，双击即可运行，不需要安装 Python：

- **Windows**:`barcode-reader-windows.zip`，解压后运行其中的 `BarcodeReader.exe`
- **macOS**:`barcode-reader-macos.zip`，解压得到 `BarcodeReader.app`；首次打开未签名应用时，在「系统设置 → 隐私与安全性」中点「仍要打开」
- **Linux**:`barcode-reader-linux.tar.gz`，解压后运行其中的 `BarcodeReader`
